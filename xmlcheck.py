...
if os.path.exists('hp_output-{}.xml'.format(host)): 
    os.remove('hp_output-{}.xml'.format(host))
    print("### Earlier HP XML results are deleted and checks will be overwritten. ###")
elif os.path.exists('dell_output-{}.xml'.format(host)):
    os.remove('dell_output-{}.xml'.format(host))
...
# finish check vendor
    ...
    def __call__(self, command):
        ...
        #stdout_outputs.append(stdout.readlines().rstrip('/n'))
...
def get_output_dict():    
    ...
    for test in data['platform']['vendor']['tests']:
        ...
        #subprocess.call(["python", "./testlink_poc/build_tools-master/tools/testlinkAPI/executeTestcase_stefan.py", "CTP", "test_hp", test['@name'], "p", "HP", "SXA", str1])
        
    '''for kljuc in data['platform']['vendor']['tests']:
        command1 = kljuc.get('question')
        output1 = remote(command1)
        str2 = ''.join(str(e) for e in output1)
        print("TEST", str2)
        #if kljuc['@equals']) == '''
...
if os.path.exists('hp_output-{}.xml'.format(host)): 
    inputfile='hp_output-{}.xml'.format(host)

elif os.path.exists('dell_output-{}.xml'.format(host)):
       inputfile='dell_output-{}.xml'.format(host)
#    print("### Earlier XML results are deleted and checks will be overwritten. ###")
else:
...
'''#https://stackoverflow.com/questions/17867583/merging-xml-files-using-element-tree-in-python
###MERGE TWO FILES
output = open('merge.xml','w')
dir = './output'
filename = 'hp_output-{}.xml'.format(host)
xml_files = glob.iglob(os.path.join(dir, '*.xml'))
xml_element_tree = None
for xml_file in xml_files:
        data = ElementTree.parse(xml_file).getroot()
        print(ElementTree.tostring(data))
        with open(filename, "wb") as xy:
        #print("ovo je host", host)
             xy.write(data)'''

        #for result in data.iter('tests'):
        #    if xml_element_tree is None:
        #        xml_element_tree = data 
        #        insertion_point = xml_element_tree.findall("platform/vendor/tests/item/*")
        #    else:
